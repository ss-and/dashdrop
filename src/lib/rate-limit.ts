 ...
 *
 * 数えているのは**IP**なので、対象は「1人」ではなく「1つの出口」。日本の
 * 会社は1本の回線の内側に全員がいるのが普通で、5回/時では**同じ事務所の
 * 6人目**が締め出される。しかも締め出しは60分だった——導入の日に半分の人が
 * 登録できず、しかも理由が本人には分からない。
 *
 * 上げても総当たりの守りは落ちない。ここで守っているのは「1つのIPから
 * 無限にアカウントを作られること」で、実際の歯止めはメール確認のほう
 * （確認していないアカウントは公開リンクを作れない）。15回/時なら
 * 20人規模の事務所が同じ日に登録しても通り、機械的な大量作成は止まる。
 *
 * 締め出しも 60分 → 15分 にする。正当な人が引っかかったときの被害は
 * 「待たされる時間」そのものなので、短いほうが良い。
 */
 ...
 * サイトからのお問い合わせ。
 *
 * 守りたいのは総当たりではなく**受信箱**。フォームは誰でも叩けるので、
 * 上限が無いと1つのIPから無限にメールを積める。運営者の受信箱が埋まると、
 * 本物の問い合わせが埋もれる——気づけないまま商談を落とす。
 *
 * 5回/時間は、書き直して出し直す人を巻き込まない範囲。
 */
