 ...
 * ## なぜ 200 では足りないのか
 *
 * 打ち切っているのは**時間ではなくこの件数**だった。実測（2026-08-30・手元）で
 * 200ワークスペースの一巡は **1.0秒**——50秒の予算のうち 2% しか使わずに、
 * 件数の上限で止まっていた。
 *
 * 止まった残りは次回に回るので、1,000社なら一巡に5回＝**75分**かかる。
 * 画面に書いてある「15分ごとに確認します」が、静かに嘘になる。しかも
 * 遅れて届くだけなので、届かないより気づきにくい。
 *
 * 実測から線形に伸ばすと 2,000件でも約10秒で、予算の50秒に対して十分収まる。
 * 本当の歯止めは時間のほう（DEFAULT_BUDGET_MS と DEFAULT_MIN_START_MS）で、
 * 間に合わなければ応答の skipped に残って次回に回る。件数の上限は
 * 「1回の応答とDB負荷が青天井にならない」ためだけに置く。
 */
export const DEFAULT_MAX_WORKSPACES = 2_000;
