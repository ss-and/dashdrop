 ...
 * for customers, this makes it the master record for people. The objects are
 * created as real Collections with typed Fields, resolved relations and
 * computed columns (lookup / rollup / formula), then optionally seeded with
 * demo rows whose links are wired to actual record ids.
 *
 * Idempotent by slug: objects that already exist in the workspace are skipped,
 * so calling this twice never duplicates the employee database.
 */
...
import { HR_OBJECTS, type HrObject } from "./hr-objects";
import type { CurrentUser } from "./auth";

export interface InstallHrResult {
...
function primaryKeyOf(obj: HrObject): string {
...
export async function installHr(
  ...
  const toCreate = HR_OBJECTS.filter((o) => !bySlug.has(o.slug));
  const skipped = HR_OBJECTS.filter((o) => bySlug.has(o.slug)).map((o) => o.slug);
  ...
  // slug -> collection id, for every HR object present after this run.
  const idBySlug = new Map<string, string>(
    HR_OBJECTS.filter((o) => bySlug.has(o.slug)).map((o) => [
      o.slug,
  ...
  try {
    ...
    // Relation/lookup/rollup/formula config is filled in pass 2, once every HR
    ...
    if (withSampleData) {
      // displayName -> recordId, per HR slug (built as we go).
    ...
    // Roll back anything this call created so a failure leaves no partial HR db.
    if (created.length > 0) {
      await db.collection
    ...
    console.error("installHr failed:", err);
    throw new ApiError("社員データベースの作成に失敗しました", 500);
  }
}
