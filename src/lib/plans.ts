...
import { WIDGET_TYPES } from "./widget-builder";
import { PALETTES } from "./palette";

export type PlanId = "free" | "pro" | "business";

/*
 * 価格表に出す「種類の数」は、**必ず実装から数える**。
 *
 * ここは2回続けて間違えた場所で、どちらも数字を手で書いていたのが原因。
 * 22 と書いてある横でウィジェットが1つ増え、直すときに数え直した対象が
 * ずれて 20 になった（棒グラフと100%積み上げを1つと数えるかどうかで
 * 答えが変わる）。手で書く限り、増やすたびに同じことが起きる。
 *
 * 「グラフ」と言い切らず「図表」にしてあるのは、表・ピボット・KPI も
 * 置けるものに含まれているから。表をグラフとは呼べない。
 */
const WIDGET_KINDS = WIDGET_TYPES.length;
...
export const PLANS: Record<PlanId, Plan> = {
  free: {
    ...
    features: [
      ...
      "取り込んだ表の編集・並べ替え・絞り込み",
      `ダッシュボードの自動作成（${WIDGET_KINDS}種類の図表・${PALETTE_COUNT}つの配色）`,
      "ダッシュボードの共有リンク",
      "Excel（.xlsx）への書き出し",
    ],
    planned: [],
