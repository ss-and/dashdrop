...
/**
 * Max upload size for a spreadsheet import.
 *
 * 【15MB → 4MB に引き下げた理由】
 * Vercel のサーバーレス関数はリクエストボディを **4.5MB** で打ち切る。この
 * 判定はプラットフォーム側にあり、ハンドラが起動する**前**に 413 を返すので、
 * ここで 15MB を許すと 4.5〜15MB のファイルでは
 *   - こちらの日本語メッセージ（「シートを分けてください」等）が一切出ない
 *   - 画面には本文の無い 413 だけが残り、利用者には原因不明のエラーになる
 * という、いちばん助けが要る場面でいちばん不親切な結果になっていた。
 *
 * 「受け付けると宣言した上限」は「自分で止められる上限」より大きくしてはいけない。
 * 4.5MB の手前で自分から断ることで、断る理由と次の一手を日本語で伝えられる。
 * 端数（0.5MB）は multipart のヘッダやフィールド境界の分の余裕として捨てる。
 *
 * 上げるときは必ずプラットフォーム側の上限も一緒に確認すること。ホスティングを
 * 変えたり、アップロードを直接ストレージに逃がしたりして 4.5MB の壁が無くなった
 * ときだけ、この値は意味を持って大きくできる。
 */
export const MAX_IMPORT_BYTES = 4 * 1024 * 1024; // 4MB
